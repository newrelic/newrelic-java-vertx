...
public class TokenUtils implements Runnable {
	...
	public static boolean distributedTracingEnabled = false;
	...
	static {
		...
		distributedTracingEnabled = config.getValue("distributed_tracing.enabled", false);
		NewRelic.getAgent().getLogger().log(Level.FINE, "distribuedTracingEnabled={0}", distributedTracingEnabled);
	}
